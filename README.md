...
По умолчанию панель работает в режиме **«Порядок: события»**: сначала пропуски одного лейбла по строкам, затем следующие ивенты того же персонажа в порядке его внутриигровой галереи. События могут находиться в разных файлах. В конце очереди переходы останавливаются: выбери другой ивент в отчёте. Общие события могут входить в несколько галерей; при навигации сохраняется выбранная очередь. При новом переходе из браузера выбирается первая подходящая галерея (персонажи перед общей и секретной).

Для лейблов вне галереи используется отдельная очередь по исходному файлу; панель явно подписывает её «Вне галереи, файл». Это не доказанная принадлежность персонажу. Кнопка **«Порядок: события»** переключает на прежний общий список и обратно. Панель показывает выбранную очередь, номер фрагмента и лейбл. Внутри фрагмента продолжай обычными кликами; «Следующий фрагмент» переходит к следующей группе пропусков.

В обоих режимах скрыты явные ветки `bonus == False`, прочитанные фрагменты пропускаются по живому persistent. **Фильтры браузера не управляют очередью.** Порядок галереи не гарантирует единственную сюжетную хронологию и не подбирает переменные. После обновления полностью перезапусти игру и заново создай отчёт; старые отчёты сохраняют прежний порядок.
...
Топик, совместимость, ресёрч - @SkyIIne_R
Реализация, html, реверс кода ренпая - GPT-6 Astra
